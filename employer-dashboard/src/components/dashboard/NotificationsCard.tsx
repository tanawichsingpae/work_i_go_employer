...
type Notice = { jobpost_id: number; title: string; status: string; created_at: string };

const NotificationsCard = () => {
  const { data, isLoading, error } = useQuery<Notice[]>({ queryKey: ["notifications"], queryFn: () => fetchJson<Notice[]>("/api/employer/notifications") });
  const { t } = useLanguage();
  const items = data || [];
  ...
  return (
    <div className="rounded-lg bg-card p-4 sm:p-5 shadow-sm border border-border">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-display font-semibold text-card-foreground">{t("notifications")}</h3>
        <span className="text-xs text-muted-foreground">{items.length}</span>
      </div>
      {isLoading && <p className="text-sm text-muted-foreground">{t("loading")}</p>}
      {error && !isLoading && <p className="text-sm text-destructive">{t("loadError")}</p>}
      {!isLoading && !error && items.length === 0 && <p className="text-sm text-muted-foreground">{t("noPendingItems")}</p>}
