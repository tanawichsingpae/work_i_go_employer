    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="rounded-lg bg-card p-6 shadow-sm border border-border">
        <div className="flex justify-between items-start mb-4">
          <h3 className="font-display font-semibold text-lg text-card-foreground">{t("companySummary")}</h3>
          {employer.verified && (
        ...
        <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground">
          <div className="bg-secondary/30 p-3 rounded-md">
            <p className="text-xs uppercase tracking-wider mb-1">{t("totalJobs")}</p>
            <p className="text-2xl font-display text-card-foreground">{employer.total_jobs}</p>
          </div>
          <div className="bg-secondary/30 p-3 rounded-md">
            <p className="text-xs uppercase tracking-wider mb-1">{t("completed")}</p>
            <p className="text-2xl font-display text-card-foreground">{employer.completed_jobs}</p>
          </div>
          <div className="bg-secondary/30 p-3 rounded-md">
            <p className="text-xs uppercase tracking-wider mb-1">{t("activeWorkers")}</p>
            <p className="text-2xl font-display text-card-foreground">{employer.active_workers}</p>
          </div>
        </div>
      </div>
      <div className="rounded-lg bg-card p-6 shadow-sm border border-border">
        <h3 className="font-display font-semibold text-lg text-card-foreground mb-4">{t("companyInfo")}</h3>
        <div className="space-y-3 text-sm">
          <div className="flex items-center text-muted-foreground">
            <Briefcase className="w-4 h-4 mr-3 shrink-0" />
            <div className="flex-1">
              <p className="text-xs mb-0.5">{t("businessNameType")}</p>
              <p className="font-medium text-card-foreground">{employer.business_name || "N/A"}{employer.business_type ? ` • ${employer.business_type}` : ""}</p>
            </div>
          </div>
          <div className="flex items-center text-muted-foreground">
            <Mail className="w-4 h-4 mr-3 shrink-0" />
            <div className="flex-1">
              <p className="text-xs mb-0.5">{t("contact")} ({employer.contact_method || "Email"})</p>
          ...
          <div className="flex items-center text-muted-foreground">
            ...
            <div className="flex-1">
              <p className="text-xs mb-0.5">{t("location")}</p>
